Template._timeline.rendered = ->
	...
	createTimeline = -> 
		...
		allData = allDebt.concat(allEquity).sort(d3.ascending)
		...
		mousemove = ->
			...
			# 	d0 = set[i - 1]
			# 	d1 = set[i]
			# 	if (x - new Date(d0.date)) > (new Date(d1.date) - x) then ix = d1 else ix = d0

			# 	if min < x < max
			# 		d3.select(this)
			# 			.style('display', null)
			# 			.attr 'x', (xScale(new Date ix.date) - box.s)
			# 			.attr 'y', (yScale(ix.amount) - box.s)
			# 	else
		...
		debtGroup = debtGroup.selectAll('.debt')
			.data(debtLines.sort(d3.ascending))
			.enter()
			.append('g')
			.attr('class', 'debt')
